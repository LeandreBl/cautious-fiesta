...
{    
    class Character
    {
        public:
           ...
           Character(const std::string &name, const struct stats &stats) noexcept
            ...
            void setLife(float life) noexcept {_stats.life = life;};
            float getLife() const noexcept {return _stats.life;};
            void setSpeed(float speed) noexcept {_stats.speed = speed;};
            float getSpeed() const noexcept {return _stats.speed;};
            void setAttack(float attack) noexcept {_stats.attack = attack;};
            float getAttack() const noexcept {return _stats.attack;};
            void setAttackSpeed(float as) noexcept {_stats.attackSpeed = as;};
            float getAttackSpeed() const noexcept {return _stats.attackSpeed;};
            void setArmor(float armor) noexcept {_stats.armor = armor;};
            float getArmor() const noexcept {return _stats.armor;};
        protected:
            std::string _name;
            struct stats _stats;
    };
    ...
    {
        public:
            CharacterStatName(const sf::Font &font, const std::string &name, const sf::Vector2f &pos) noexcept
                : _text(nullptr)
            {
                setPosition(pos);
                _text = &addComponent<sfs::Text>(font, name, sf::Color::White, 18);
            }
    ...
    class CharacterCreation : public sfs::UI
    {
        public:
            CharacterCreation() noexcept
                : _box(nullptr), _printTotal(nullptr)
                {};
            void start(sfs::Scene &scene) noexcept
            {
                scene.subscribe(*this, sf::Event::KeyPressed);
                _box = &addChild<sfs::InputBox>(scene, *scene.getAssetFont("assets/fonts/commodore-64.ttf"), sf::Vector2f(0, 0), "[name]", sf::Color::White, 20);
                _box->addComponent<PadderH<sfs::InputBox>>(56, *_box);
                _box->addComponent<PadderW<sfs::InputBox>>(180, *_box);
                float height = 23;
                for (int i = 0; i < 5; i += 1) {
                    auto inputBox = &addChild<sfs::CustomBox>(scene, *scene.getAssetFont("assets/fonts/commodore-64.ttf"), sf::Vector2f(0, 0), "[]", sf::Color::White, 18, "0123456789");
                    inputBox->addComponent<PadderH<sfs::CustomBox>>(height, *inputBox);
                    inputBox->addComponent<PadderL<sfs::CustomBox>>(910, *inputBox);
                    _stats.emplace_back(inputBox);
                    height += 18;
                }
                _printTotal = &addChild<CharacterStatName>(scene, *scene.getAssetFont("assets/fonts/commodore-64.ttf"), "total :", sf::Vector2f(950, 600));
            }
            void update(sfs::Scene &scene[[gnu::unused]]) noexcept
            {
                float total = 100;
                for (auto &i : _stats) {
                    if (i->string() != ""){
            ...
            void onEvent(sfs::Scene &scene[[gnu::unused]], const sf::Event &event) noexcept {
