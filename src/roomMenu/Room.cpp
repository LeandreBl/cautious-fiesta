...
void Room::start(sfs::Scene &scene) noexcept
{
	...
	_roomName =
		&addComponent<sfs::Text>(*_font, "", sf::Color::White, 50, sf::Vector2f(200, 40));

	_toggleReady = &addChild<sfs::Button>(
		scene, *texture2, *_font, sf::Vector2f(-1000, -1000),
		std::bind(&Room::togglePlayerReadyState, this, std::ref(scene)), "ready",
		sf::Color::White, 20);
	auto pos =
		(_image->getGlobalBounds().width / 2) - (_toggleReady->getGlobalBounds().width / 2);
	_toggleReady->addComponent<sfs::Offset>(this->getPosition(), sf::Vector2f(pos, 150));

	_players =
		&addComponent<sfs::Text>(*_font, "", sf::Color::White, 50, sf::Vector2f(200, 300));
