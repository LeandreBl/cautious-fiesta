...
optionSound::optionSound() noexcept
	: _soundText(nullptr)
	, _decreaseSound(nullptr)
	, _augmentSound(nullptr)
	, _soundBar(nullptr){};

void optionSound::start(sfs::Scene &scene) noexcept
{
	_soundText =
		&addComponent<sfs::Text>(*scene.getAssetFont("local-assets/fonts/commodore-64.ttf"),
					 "SOUND", sf::Color::White, 30);
	_decreaseSound = &addChild<sfs::Button>(
		scene, *scene.getAssetTexture("local-assets/sprites/Menu/ui/ArrowButton.png"),
		*scene.getAssetFont("local-assets/fonts/commodore-64.ttf"),
		sf::Vector2f(-100, -100),
		std::bind(&optionSound::lowerSound, this, std::ref(scene)));
	...
	_augmentSound = &addChild<sfs::Button>(
		...
		*scene.getAssetFont("local-assets/fonts/commodore-64.ttf"),
		...
		std::bind(&optionSound::augmentSound, this, std::ref(scene)));
	...
	_soundBar = &addComponent<sfs::Rectangle>(
		sf::Vector2f(-16, 50), sf::Vector2f((150 * volume) / 100, 30), sf::Color::White);
}
